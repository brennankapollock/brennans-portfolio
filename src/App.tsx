...
function AppContent() {
  ...
  const isScrollable = location.pathname.includes('/poems/') || 
                      location.pathname === '/projects' ||
                      location.pathname === '/songs' ||
                      location.pathname === '/lastfm';

  return (
    <div className={`min-h-screen bg-white font-mono ${!isScrollable ? 'overflow-hidden' : ''}`}>
      {/* Mobile menu button */}
      <button 
        onClick={toggleMenu}
      ...
      <main className={`ml-0 md:ml-16 p-4 md:p-8 ${!isScrollable ? 'h-screen overflow-hidden' : ''}`}>
